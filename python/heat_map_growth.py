...
df = pd.read_csv('spatial_difference_summary.csv', usecols=[
                 'state', 'abb', 'rho', 'phi', 'theta', 'total', 'rho_per', 'phi_per', 'theta_per', 'latitude', 'longitude', 'growth'])
...
dgrowth ={k: float(v) for k,v in df.groupby("state")["growth"]}
...
cmap = plt.cm.cool # use 'hot' colormap plt.cm.bwr #
...
for shapedict in m.states_info:
    ...
    if statename in include: #not in exclusions: #['District of Columbia','Puerto Rico']:
        growths = dgrowth[statename]
        # calling colormap with value between 0 and 1 returns
        # rgba value.  Invert color range (hot colors are high
        # population), take sqrt root to spread out colors more.
        # color_value = 1.-np.sqrt((growths-vmin)/(vmax-vmin))
        color_value = (growths-growth_min)/(growth_max-growth_min)
...
p = PatchCollection(patches, cmap=plt.cm.cool, alpha=0.6)
# p.set_array(np.array(A))
# p.set_array(np.array(color_values))
re_color = np.array(color_values) * (growth_max-growth_min)  + growth_min
