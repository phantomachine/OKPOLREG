...
abbs = df['abb'].values
percentages = df['theta_per'].values
...
for shapedict in m.states_info:
    ...
    if statename in include: #not in exclusions: #['District of Columbia','Puerto Rico']:
        thetas = dtheta[statename]
        # calling colormap with value between 0 and 1 returns
        # rgba value.  Invert color range (hot colors are high
        # population), take sqrt root to spread out colors more.
        # color_value = 1.-np.sqrt((thetas-vmin)/(vmax-vmin))
        if statename == 'Maryland':
            thetas = theta_max
        color_value = (thetas-theta_min)/(theta_max-theta_min)
...
re_color = np.array(color_values) * (theta_max-theta_min)  + theta_min
