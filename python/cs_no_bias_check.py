...
def solve_phi_k_3D(phi_ks): #y={eta_fixed, mu_fixed, nu_fixed}
	...
	for phi_k in phi_ks:
		...
		L_s = N * (mu_s * (1-mu_s/2) + theta * (1-mu_s))

		sol_5 = beta * (1-tau_k) * (1+alpha*A*eta_s**(alpha-1)*L_s**(1-alpha) - delta) / (1+beta) * ( (1-tau_l)* (1-mu_s)* (1-alpha)* A* eta_s**(alpha-1)* L_s**(-alpha) - gamma)

		sol_6 = 1/(N*(1-mu_s)) * (tau_l*(1-alpha)*A*eta_s**(alpha-1)*L_s**(1-alpha) + tau_k* (alpha*A*eta_s**(alpha-1)*L_s**(1-alpha) +N*mu_s*gamma) - eta_s**(-1))
		...
		if r.stopcase>0:
			...
			sol_5_array.append(sol_5)				
			sol_6_array.append(sol_6)				

	return phi_k_array, sol_1_array, sol_2_array, sol_3_array, sol_4_array, sol_5_array, sol_6_array
...
phi_ks = np.linspace(0.8* phi_l, 1.2*phi_l, num=20)

phi_x, phi_eta, phi_mu, phi_nu, phi_L, phi_ok, phi_ol  = solve_phi_k_3D(phi_ks)
...
# axes[3,0].set_xlabel(r'$\rho$', fontsize=f_size*1.5)
...
# axes[3,1].set_xlabel(r'$\phi^{*k}(0) / \phi^{*l}(0) $', fontsize=f_size*1.5)
...
# axes[3,2].set_xlabel(r'$\theta$', fontsize=f_size*1.5)
