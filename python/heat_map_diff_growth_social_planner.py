...
dnu ={k: float(v) for k,v in df.groupby("state")["nu"]}
...
for shapedict in m.states_info:
    ...
    if statename in include: #not in exclusions: #['District of Columbia','Puerto Rico']:
        ...
        # color_value = 1.-np.sqrt((nus-vmin)/(vmax-vmin))
        color_value = (nus-nu_min)/(nu_max-nu_min)
...
re_color = np.array(color_values) * (nu_max-nu_min)  + nu_min
